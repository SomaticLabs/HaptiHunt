...
//  GLfloat _cube_position_x; // FIXME?
//  GLfloat _cube_position_y; // FIXME?
//  GLfloat _cube_position_z; // FIXME?
...
- (void)renderWithModelViewProjectionMatrix:(const float *)model_view_matrix {
  ...
  _cube_position[0] = _cube_position_x;
  _cube_position[1] = _cube_position_y;
  _cube_position[2] = _cube_position_z;
...
- (void)setRandomCubePosition:(float)min maxLimit:(float)max {
  ...
  _cube_position_x = -cos(elevation) * sin(azimuth) * distance;
  _cube_position_y = sin(elevation) * distance;
  _cube_position_z = -cos(elevation) * cos(azimuth) * distance;
}
